...
We observe that $z$ is an eigenfunction of $D$. Furthermore, since $D$ is linear, any linear combination of eigenfunctions of $D$ is also an eigenfunction of $D$. Solutions of this equation form a vector space.

From thousands of years of human knowledge, people told me that one of the eigenfunction of $D$ is $e^{\alpha t}$ for $\alpha \in \mathbb{R}$. It's simple to verify

\begin{equation}
    De^{\alpha t} = \alpha e^{\alpha t}
\end{equation}

So the associated eigenvalue is $\alpha$.

Lucky for us, many functions can be written as a finite sum/infinite sum of $c e^{\alpha t}$ \footnote{How many? I don't know. Maybe related to equi-small tails property of subset of Hilbert space w.r.t countable set of $e^{\alpha t}$}. Write $x$ as an infinite sum of $c e^{\alpha t}$

\begin{equation}
    (D - r_1) z = \sum_{i=1}^\infty c_i e^{\alpha_i t}
\end{equation}

Write $z$ as an infinite sum of $c e^{\alpha t}$ with the same "basis"

\begin{align*}
    (D - r_1) \sum_{i=1}^\infty d_i e^{\alpha_i t}  &= \sum_{i=1}^\infty c_i e^{\alpha_i t} \\
    \sum_{i=1}^\infty (\alpha_i - r_1) d_i  e^{\alpha_i t}  &= \sum_{i=1}^\infty c_i e^{\alpha_i t} \\
\end{align*}

So, we can calculate $d_i = c_i / (\alpha_i - r_1)$

Since we can solve \ref{base_case}, we can solve the $n$-order differential equation with constant coefficients using induction argument.
